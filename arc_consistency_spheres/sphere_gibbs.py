...
# np.random.seed(0)
placement_times = np.zeros([WORLD_WIDTH]*2)
step = 0
...
probmap = np.ones((WORLD_WIDTH,WORLD_WIDTH, len(tiles))).astype(np.float32) / len(tiles)
...
def generate_world():
    ...
    while np.prod(decided.shape) - np.sum(decided) > 0:
        step += 1
        place_a_tile()
        if not np.all(np.sum(probmap, axis=-1) > 0):
            break
...
# print '=' * WORLD_WIDTH * 3
# draw_world(world, tiles, mask = decided)
print get_number_of_errors(), "invalid tiles"
# fix_world()
# print get_number_of_errors(), "invalid tiles after fixup"
draw_world(world, tiles, mask = decided)
